...
self.addEventListener('fetch', (event) => {
  ...
  // ─── FILTRO CRÍTICO: só processar requisições http/https ───
  // A Cache API não suporta esquemas como chrome-extension://, moz-extension://,
  // safari-web-extension://, data:, blob:, etc. Tentar cachear esses esquemas
  // dispara TypeError e quebra a cadeia de Promises do SW.
  // Extensões do navegador (AdBlock, gerenciadores de senha, etc.) podem injetar
  // tais requisições na página — precisam ser ignoradas pelo SW.
...
async function cacheFirst(request, cacheName) {
  ...
  try {
    ...
    if (response && response.status === 200 && response.type !== 'opaque') {
      // Defensividade: cache.put pode falhar para alguns esquemas/recursos.
      // Falha de cache não deve quebrar a resposta.
      try {
        const cache = await caches.open(cacheName);
        await cache.put(request, response.clone());
      } catch (cacheErr) {
        // Silencioso: a resposta da rede continua válida mesmo se o cache falhou.
      }
    }
    return response;
  } catch (err) {
    ...
    .then(async (response) => {
      if (response && response.status === 200) {
        ...
        try {
          ...
          // Silencioso: a resposta continua válida mesmo se o cache falhou.
        }
      }
      return response;
    })
